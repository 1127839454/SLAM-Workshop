...
class SickTIM310:
    ...
    def get2DMapping(self):       
        ...
        l = len(self.lidar)//3
        x = [self.lidar_data[3 * i] for i in range(l)]
        y = [self.lidar_data[3 * i + 1] for i in range(l)]
        return x, y
    ...
    def getAllDistances(self):
        distances = []
        self.lidar_data = self.sim.callScriptFunction('get_data', self.script_handle)
        temp = len(self.lidar_data)
        print('Length of the lidar data list: ', temp)
        l = temp//3
        for i in range(l): #may sometimes be less than 270
